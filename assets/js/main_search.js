...
function getDataBook() {
    ...
    http.open('GET', path + `api/get-databook.php?page=${page}&size=${size}&search=${txtSearch}&cate=${cate}`, true);
...
function loadPaginationToView(numberBook) {
    ...
    pageDOM.innerHTML = `
    <a href="search.html?page=1&size=${size}&search=${txtSearch}&cate=${cate}" class="app-content-pagination__first-page">Về trang đầu</a>
    <a href="search.html?page=${(page - 1 > 1) ? page - 1 : 1}&size=${size}&search=${txtSearch}&cate=${cate}" class="app-content-pagination__before-page"><</a>
    `;
    ...
    for(let i = from; i <= to; i++) {
        pageDOM.innerHTML += `
        <a href="search.html?page=${i}&size=${size}&search=${txtSearch}&cate=${cate}" class="app-content-pagination__item-page">${i}</a>
        `;
        if(i == numberPage) break;
    }
    if(to < numberPage) {
        pageDOM.innerHTML += `
        <a href="search.html?page=${numberPage}&size=${size}&search=${txtSearch}&cate=${cate}" class="app-content-pagination__item-page">...</a>
        `;
    }

    pageDOM.innerHTML += `
    <a href="search.html?page=${(page + 1 < numberPage) ? page + 1 : numberPage}&size=${size}&search=${txtSearch}&cate=${cate}" class="app-content-pagination__before-page">></a>
    <a href="search.html?page=${numberPage}&size=${size}&search=${txtSearch}&cate=${cate}" class="app-content-pagination__last-page">Về trang cuối</a>
    `;
